...
public class BluetoothManager
{
	...
	public void changeContext(Context context, BluetoothListener blueToothListener)
	{
		this.context = context;
		this.blueToothListener = blueToothListener;
	}
	...
	private BroadcastReceiver mReceiver = new BroadcastReceiver()
	{
		...
		public void onReceive(Context context, Intent intent)
		{
			...
			}else if(BluetoothAdapter.ACTION_DISCOVERY_FINISHED.equals(action))
			{
				...
				{
					blueToothListener.optionCallBack(BluetoothListener.SearchFinished, null);
				}
	...
	public void connect(int position, String psw)
	{
		try
		{
			...
			{
				this.blueToothListener.optionCallBack(BluetoothListener.BluetoothConnected, null);
			}
		}catch(IOException e)
		{
			error("Bluetooth connect failed !!!");
...
//					for (byte b : data) {
//						System.out.println("id:"+id+";b:"+Integer.toHexString(b));
//					}
					os.write(data);
					os.flush();
					Thread.sleep(50);
					...
					byte[] isData = new byte[length];  //data buffer
					byte[] result=new byte[length+3];
					int readDataLength = 0;
					int index=0;
					while((readDataLength=is.read(isData))>0)
					{
						for (int i = 0; i < readDataLength; i++)
						{
//							System.out.println("index::"+index+";isData[i]::"+isData[i]);
